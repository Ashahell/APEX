...
use crate::unified_config::user_constants::{MAX_PREFERRED_TOOLS, MAX_PREFERRED_CATEGORIES};
...
impl CommunicationStyle {
    ...
    pub fn system_prompt_prefix(&self) -> &'static str {
        match self {
            ...
            CommunicationStyle::Casual => "Feel free to be conversational and friendly in responses.",
            CommunicationStyle::Technical => "Provide detailed technical explanations with code examples when appropriate.",
            CommunicationStyle::Concise => "Keep responses brief and to the point. Prioritize clarity over detail.",
        }
    }
}
...
impl UserProfileManager {
    ...
    pub async fn set_communication_style(&self, style: CommunicationStyle) -> Result<(), UserProfileError> {
        {
            let mut profile = self.profile.write().await;
    ...
    pub async fn set_response_format(&self, format: ResponseFormat) -> Result<(), UserProfileError> {
        {
            let mut profile = self.profile.write().await;
    ...
    pub async fn get_system_prompt_additions(&self) -> String {
        ...
        // Communication style
        additions.push(profile.communication_style.system_prompt_prefix().to_string());
        ...
        match profile.response_format {
            ResponseFormat::Markdown => additions.push("Use markdown formatting for better readability.".to_string()),
            ResponseFormat::Structured => additions.push("Structure responses with clear headers and bullet points.".to_string()),
            ResponseFormat::PlainText => additions.push("Use plain text without markdown formatting.".to_string()),
        }
...
mod tests {
    ...
    fn test_communication_style_system_prompt() {
        assert!(CommunicationStyle::Formal.system_prompt_prefix().contains("formal"));
        assert!(CommunicationStyle::Casual.system_prompt_prefix().contains("conversational"));
        assert!(CommunicationStyle::Technical.system_prompt_prefix().contains("technical"));
        assert!(CommunicationStyle::Concise.system_prompt_prefix().contains("brief"));
    }
