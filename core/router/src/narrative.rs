use apex_memory::{MemoryStats, NarrativeConfig, NarrativeEntry, NarrativeMemory};
use std::sync::Arc;
...
impl NarrativeService {
    pub async fn new(config: NarrativeConfig) -> Result<Self, apex_memory::MemoryError> {
        ...
        memory
            .initialize()
            .await
            .map_err(|e| apex_memory::MemoryError::Narrative(e.to_string()))?;
    ...
    fn extract_lessons(&self, status: &str, tools_used: &[String]) -> Vec<String> {
        ...
        if tools_used.is_empty() && status == "completed" {
            lessons
                .push("Task completed without tool usage - possibly a direct response".to_string());
        }

        if tools_used.len() > 5 {
            lessons.push(format!(
                "Complex task requiring {} tool invocations",
                tools_used.len()
            ));
        }
    ...
    pub async fn add_reflection(
        ...
        title: &str,
        content: &str,
    ) -> Result<std::path::PathBuf, apex_memory::MemoryError> {
        let memory = self.memory.read().await;
        memory
            .add_reflection(title, content)
            .await
            .map_err(|e| apex_memory::MemoryError::Narrative(e.to_string()))
    }
    ...
    pub async fn add_entity(
        ...
        let memory = self.memory.read().await;
        memory
            .add_entity(entity_type, name, content)
            .await
            .map_err(|e| apex_memory::MemoryError::Narrative(e.to_string()))
    }
    ...
    pub async fn add_knowledge(
        ...
        let memory = self.memory.read().await;
        memory
            .add_knowledge(category, title, content)
            .await
            .map_err(|e| apex_memory::MemoryError::Narrative(e.to_string()))
    }
