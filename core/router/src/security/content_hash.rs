...
impl ContentHash {
    ...
    pub fn normalize_path(base_path: &Path, target_path: &Path) -> Result<PathBuf, ContentHashError> {
        // Resolve both paths to absolute, canonical form
        let base = base_path.canonicalize()
            .map_err(|e| ContentHashError::Normalization(format!("Failed to canonicalize base: {}", e)))?;
        ...
        let canonical = target.canonicalize()
            .map_err(|e| ContentHashError::Normalization(format!("Failed to canonicalize target: {}", e)))?;
        ...
        #[cfg(windows)]
        {
            ...
            if !canonical_str.starts_with(&base_str) {
                return Err(ContentHashError::Normalization(
                    format!("Path escapes base directory: {} not in {}", canonical.display(), base.display())
                ));
            }
        }
