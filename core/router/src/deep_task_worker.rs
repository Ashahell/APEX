use apex_memory::decision_journal::{CreateDecisionEntry, DecisionJournalRepository};
...
impl DeepTaskWorker {
    ...
    async fn process_deep_task(&self, message: DeepTaskMessage) {
        ...
        match result {
            Ok(output_json) => {
                ...
                let journal_entries =
                    Self::extract_journal_entries(&message.task_id, &final_output);

                // Write narrative to long-term memory
                let tools_used: Vec<String> = output_val
                    .get("history")
                    .and_then(|h| h.as_array())
                    .map(|arr| {
                        arr.iter()
                            .filter_map(|s| {
                                s.get("action").and_then(|a| a.as_str()).map(String::from)
                            })
                            .collect()
                    })
                    .unwrap_or_default();
            ...
            if let Some(mut rx) = execution_streams.subscribe(&task_id_for_spawn) {
                while let Ok(event) = rx.recv().await {
                    if let Ok(json) = serde_json::to_string(&event) {
                        ws_manager
                            .broadcast_task_update(&task_id_for_spawn, &json)
                            .await;
                    }
                }
            }
    ...
    fn generate_skill_name(&self, task_content: &str) -> String {
        ...
        if words.is_empty() {
            format!(
                "task-{}",
                &task_content[..task_content.len().min(20)].to_lowercase()
            )
        } else {
    ...
    fn is_common_word(word: &str) -> bool {
        let common = [
            "the", "and", "for", "with", "this", "that", "from", "have", "been", "were", "they",
            "what", "when", "where", "which", "about", "into", "make", "just", "also", "very",
            "some", "could", "would", "should",
        ];
    ...
    fn generate_skill_content(
        ...
        let action_summary = task_content.lines().take(3).collect::<Vec<_>>().join(" ");
