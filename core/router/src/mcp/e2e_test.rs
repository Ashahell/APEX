...
mod e2e_tests {
    ...
    async fn test_mcp_e2e_full_flow() {
        ...
        if !server_path.exists() {
            println!(
                "Test server not found at {:?}, skipping E2E test",
                server_path
            );
            return;
        }
        ...
        let echo_result = client
            .call_tool("echo", serde_json::json!({"message": "Hello from test!"}))
            .await;
        assert!(
            echo_result.is_ok(),
            "Failed to call echo: {:?}",
        ...
        assert!(
            echo.content.contains("Hello from test!"),
            "Should echo message"
        );
        ...
        // Test get_time tool
        let time_result = client.call_tool("get_time", serde_json::json!({})).await;
        assert!(
            time_result.is_ok(),
            "Failed to call get_time: {:?}",
            time_result.err()
        );
    ...
    async fn test_mcp_e2e_retry_on_failure() {
        ...
        let result = client
            .connect("nonexistent-mcp-server-xyz", vec![], HashMap::new())
            .await;
