...
impl SkillManager {
    ...
    fn references_dir(&self, name: &str) -> PathBuf {
        self.auto_created_dir().join(name).join(SKILL_REFERENCES_DIR)
    }
    ...
    fn parse_skill_metadata(&self, path: &Path) -> Result<SkillMetadata, SkillError> {
        ...
        let description = Self::extract_frontmatter_value(&frontmatter, "description")
            .unwrap_or_default();
        ...
        let _tags = Self::extract_frontmatter_section(&frontmatter, "tags")
            .unwrap_or_default();
    ...
    fn extract_frontmatter_section(frontmatter: &str, key: &str) -> Option<String> {
        ...
        let start = lines.iter().position(|l| l.contains(&format!("{}:", key)))?;
        let end = lines[start..].iter().position(|l| l.trim().starts_with('-') && !l.contains(':'))? + start;
    ...
    pub async fn create_skill(&self, req: SkillCreateRequest) -> Result<SkillMetadata, SkillError> {
        ...
        Ok(SkillMetadata {
            ...
            category: req.category.unwrap_or_else(|| AUTO_SKILL_CATEGORY.to_string()),
    ...
    fn generate_skill_md(&self, req: &SkillCreateRequest) -> Result<String, SkillError> {
        let description = req.description.as_deref().unwrap_or("Auto-created skill from task execution");
        let category = req.category.as_deref().unwrap_or(AUTO_SKILL_CATEGORY);
        let source_task_id_line = req.source_task_id
            .as_ref()
...
"#,
    ...
    fn is_valid_skill_name(name: &str) -> bool {
        !name.is_empty()
            && name.len() <= 50
            && name.chars().all(|c| c.is_ascii_lowercase() || c.is_numeric() || c == '-' || c == '_')
            && !name.starts_with('-')
            && !name.starts_with('_')
            && !name.ends_with('-')
            && !name.ends_with('_')
    ...
    pub async fn patch_skill(&self, name: &str, patch: SkillPatchRequest) -> Result<(), SkillError> {
        ...
        if !skill_path.exists() {
            return Err(SkillError::NotFound { name: name.to_string() });
        }
...
mod tests {
    ...
    fn test_dangerous_patterns() {
        ...
        assert!(!SkillManager::contains_dangerous_patterns("curl https://example.com"));
