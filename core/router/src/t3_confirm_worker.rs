...
impl T3ConfirmationWorker {
    ...
    pub async fn run_supervised(mut self, worker_name: &str) {
        loop {
            tracing::info!(worker = %worker_name, "Starting supervised T3 confirmation worker");
            
            let result = self.run_inner().await;
            
            match result {
                Ok(()) => {
                    tracing::info!(worker = %worker_name, "T3 confirmation worker exited normally");
                    break;
                }
                Err(e) => {
                    tracing::error!(worker = %worker_name, error = %e, "T3 confirmation worker crashed, restarting in 1 second...");
