...
async fn registry_lifecycle_and_tools_persistence() {
    ...
    sqlx::query("INSERT INTO mcp_registries (id, name) VALUES ('reg1', 'Test Registry')")
        .execute(&pool).await.unwrap();
    let rows = sqlx::query("SELECT id, name FROM mcp_registries").fetch_all(&pool).await.unwrap();
    ...
    sqlx::query("INSERT INTO mcp_tools_registry (id, registry_id, name, description, input_schema) VALUES ('t2', 'reg1', 'dynamic_tool_2', 'Another discovered tool', '{\"type\":\"object\"}')")
        .execute(&pool).await.unwrap();
    let tools = sqlx::query("SELECT name, description, input_schema FROM mcp_tools_registry WHERE registry_id = 'reg1'").fetch_all(&pool).await.unwrap();
    ...
    sqlx::query("DELETE FROM mcp_tools_registry WHERE registry_id = 'reg1'")
        .execute(&pool).await.unwrap();
    let existing: Vec<(String,)> = sqlx::query_as::<sqlx::Sqlite, (String,)>("SELECT id FROM mcp_tools_registry WHERE registry_id = 'reg1'").fetch_all(&pool).await.unwrap();
    ...
    sqlx::query("INSERT INTO mcp_tools_registry (id, registry_id, name, description, input_schema) VALUES (?, ?, ?, ?, ?)")
        ...
        .execute(&pool).await.unwrap();
    let seeds = sqlx::query("SELECT name FROM mcp_tools_registry WHERE registry_id = 'reg1'").fetch_all(&pool).await.unwrap();
